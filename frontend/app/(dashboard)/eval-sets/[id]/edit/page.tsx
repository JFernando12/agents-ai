...
import { useEvalSet, useUpdateEvalSet } from '@/lib/hooks/useEvalSets';
...
export default function EditEvalSetPage({
  params,
}: {
  ...
  const updateMutation = useUpdateEvalSet(id);
  ...
  function updateItem(itemId: string, field: keyof EvalSetItem, value: string) {
    setItems((prev) =>
      ...
      <div className="bg-white dark:bg-[#18181B] rounded-xl border border-gray-200 dark:border-white/[0.08] h-full flex flex-col overflow-hidden">
        <PageHeader
          crumbs={[
            ...
            { label: evalSet.name, href: `/eval-sets/${id}` },
            { label: 'Editar' },
          ]}
        />
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Nombre del eval set"
                  required
                />
              </div>
              <div>
          ...
          <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-gray-100 dark:border-white/[0.06] flex-shrink-0">
            <button
              type="button"
              onClick={() => router.push(`/eval-sets/${id}`)}
